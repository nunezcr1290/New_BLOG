...
def get_all_posts():
    ...
    if current_user.is_authenticated:
        admin_ = current_user.id
    return render_template("index.html", all_posts=posts,
                           logged_in=current_user.is_authenticated, admin_user=admin_)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if this_user:
            flash("You already signed up! - Please Login instead")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if this_user:
            #check password
            if check_password_hash(this_user.password, form.password.data):
